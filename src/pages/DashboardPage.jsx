...
// Component Lịch đã được nâng cấp với hiệu ứng hover
...
const DashboardPage = () => {
    ...
    const [stats, setStats] = useState({ meetingsThisMonth: 0, totalUsers: 0 });
        ...
        const fetchDashboardData = async () => {
            try {
                const [meetingsResponse, statsResponse] = await Promise.all([
                    ...
                    (user.role === 'Admin' || user.role === 'Secretary') ? apiClient.get('/dashboard/stats') : Promise.resolve({ data: {} })
                ]);
    ...
    return (
        <div className="space-y-8">
            ...
            {loading ? <p>Đang tải dữ liệu dashboard...</p> : (
                (user?.role === 'Admin' || user?.role === 'Secretary') ? <AdminSecretaryDashboard /> : <AttendeeDashboard />
            )}
        </div>
    );
};
